...
class PackageSelectionView(APIView):
    ...
    def post(self, request, pk):
        ...
        selection, created = PackageSelection.objects.get_or_create(user=request.user, package=package)
        if created:
            return Response({'detail': 'Package selected successfully.'}, status=status.HTTP_201_CREATED)
        else:
            return Response({'detail': 'Package already selected.'}, status=status.HTTP_200_OK)
